  ...
  class GitAccess
    ...
    def initialize(path, page_file_dir = nil, bare = false)
      ...
      begin
        ...
        @branch_collection = Rugged::BranchCollection.new(@repo)
    ...
    def commit(ref)
      if sha?(ref)
        ...
        if sha = get_cache(:ref, ref)
          ...
          if cm = commit!(ref)
            set_cache(:ref, ref, cm.oid)
            set_cache(:commit, cm.oid, cm)
          end
        end
      end
    end
    ...
    def branches
      @branch_collection.each(:local).collect{|b|b}
    end

    def branch_names
      @branch_collection.each_name(:local).collect{|b|b}
    end
