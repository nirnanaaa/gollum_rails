/**
 * unminified source from https://raw.github.com/github/gollum/master/lib/gollum/frontend/public/gollum/javascript/editor/gollum.editor.js
 * 
 * gollum.editor.js A jQuery plugin that creates the Gollum Editor.
 * 
 * Usage: $.GollumEditor(); on DOM ready.
 */
(function($) {
	...
	$.GollumEditor = function(IncomingOptions) {
		...
		if (EditorHas.baseEditorMarkup()) {
			...
			if (EditorHas.editSummaryMarkup()) {
				$.GollumEditor.Placeholder
						.add($('#gollum-editor-edit-summary input'));
			...
			if (EditorHas.collapsibleInputs()) {
				$(
						'#gollum-editor .collapsed a.button, '
								+ '#gollum-editor .expanded a.button').click(
						function(e) {
							e.preventDefault();
			...
			if (EditorHas.previewButton()) {
				var formAction = $('#gollum-editor #gollum-editor-preview')
						.click(
								function() {
									// make a dummy form, submit to new target
									// window
									// get form fields
									var oldAction = $('#gollum-editor form')
											.attr('action');
									var $form = $($('#gollum-editor form').get(
											0));
			...
			if (EditorHas.functionBar()) {
				...
				var htmlSetMarkupLang = $('#gollum-editor-body').attr(
						'data-markup-lang');
				...
				if (EditorHas.formatSelector()) {
					FormatSelector
							.init($('#gollum-editor-format-selector select'));
				}
	...
	/**
	 * $.GollumEditor.defineLanguage Defines a set of language actions that
	 * Gollum can use. Used by the definitions in langs/ to register language
	 * definitions.
	 */
	$.GollumEditor.defineLanguage = function(language_name, languageObject) {
		if (typeof languageObject == 'object') {
			...
			debug('GollumEditor.defineLanguage: definition for '
					+ language_name + ' is not an object');
		}
	};

	/**
	 * debug Prints debug information to console.log if debug output is enabled.
	 * 
	 * @param mixed
	 *            Whatever you want to dump to console.log
	 * @return void
	 */
	var debug = function(m) {
		if (ActiveOptions.Debug && typeof console != 'undefined') {
			console.log(m);
		}
	};

	/**
	 * LanguageDefinition Language definition file handler Loads language
	 * definition files as necessary.
	 */
	var LanguageDefinition = {
		...
		define : function(name, definitionObject) {
			...
			if (typeof $.GollumEditor.WikiLanguage == 'object') {
				var definition = {};
				$.extend(definition, $.GollumEditor.WikiLanguage,
						definitionObject);
		...
		setActiveLanguage : function(name) {
			// On first load _ACTIVE_LANG.length is 0 and evtChangeFormat isn't
			// called.
			if (LanguageDefinition._ACTIVE_LANG != null
					&& LanguageDefinition._ACTIVE_LANG.length <= 0) {
			...
			if (!LanguageDefinition.isLoadedFor(name)) {
				...
				LanguageDefinition.loadFor(name,
						function(x, t) {
							if (t != 'success') {
								debug('Failed to load language definition for '
										+ name);
								// well, fake it and turn everything off for
								// this one
								LanguageDefinition.define(name, {});
							}

							// update features that rely on the language
							// definition
							if (EditorHas.functionBar()) {
								FunctionBar.refresh();
							}
							...
							if (LanguageDefinition
									.getHookFunctionFor("activate")) {
								LanguageDefinition.getHookFunctionFor(
										"activate")();
							}
		 ...
		 * @param string
		 *            specified_lang The language to pull a definition for.
		...
		/**
		 * loadFor Asynchronously loads a definition file for the current
		 * markup. Definition files are necessary to use the code editor.
		 * 
		 * @param string
		 *            markup_name The markup name you want to load
		 * @return void
		 */
		loadFor : function(markup_name, on_complete) {
			// Keep us from hitting 404s on our site, check the definition
			// blacklist
			...
			var script_uri = baseUrl + '/javascript/editor/langs/'
					+ markup_name + '.js';
			$.ajax({
				url : script_uri,
				dataType : 'script',
		...
		/**
		 * isLoadedFor Checks to see if a definition file has been loaded for
		 * the specified markup language.
		 * 
		 * @param string
		 *            markup_name The name of the markup.
		 * @return boolean
		 */
		...
		isValid : function() {
			return (LanguageDefinition._ACTIVE_LANG && typeof LanguageDefinition._LANG[LanguageDefinition._ACTIVE_LANG] == 'object');
		}

	};

	/**
	 * EditorHas Various conditionals to check what features of the Gollum
	 * Editor are active/operational.
	 */
	var EditorHas = {

		/**
		 * EditorHas.baseEditorMarkup True if the basic editor form is in place.
		 * 
		 * @return boolean
		 */
		baseEditorMarkup : function() {
			return ($('#gollum-editor').length && $('#gollum-editor-body').length);
		},

		/**
		 * EditorHas.collapsibleInputs True if the editor contains collapsible
		 * inputs for things like the sidebar or footer, false otherwise.
		 * 
		 * @return boolean
		 */
		...
		/**
		 * EditorHas.formatSelector True if the editor has a format selector
		 * (for switching between language types), false otherwise.
		 * 
		 * @return boolean
		 */
		...
		/**
		 * EditorHas.functionBar True if the Function Bar markup exists.
		 * 
		 * @return boolean
		 */
		functionBar : function() {
			return (ActiveOptions.HasFunctionBar && $('#gollum-editor-function-bar').length);
		},

		/**
		 * EditorHas.ff4Environment True if in a Firefox 4.0 Beta environment.
		 * 
		 * @return boolean
		 */
		...
		/**
		 * EditorHas.editSummaryMarkup True if the editor has a summary field
		 * (Gollum's commit message), false otherwise.
		 * 
		 * @return boolean
		 */
		...
		/**
		 * EditorHas.help True if the editor contains the inline help sector,
		 * false otherwise.
		 * 
		 * @return boolean
		 */
		help : function() {
			return ($('#gollum-editor #gollum-editor-help').length && $('#gollum-editor #function-help').length);
		},

		/**
		 * EditorHas.previewButton True if the editor has a preview button,
		 * false otherwise.
		 * 
		 * @return boolean
		 */
		...
		/**
		 * EditorHas.titleDisplayed True if the editor is displaying a title
	...
	var FunctionBar = {
		...
		/**
		 * FunctionBar.activate Activates the function bar, attaching all click
		 * events and displaying the bar.
		 * 
		 */
		activate : function() {
		...
		/**
		 * FunctionBar.evtFunctionButtonClick Event handler for the function
		 * buttons. Traps the click and executes the proper language action.
		 * 
		 * @param jQuery.Event
		 *            jQuery event object.
		 */
		...
		/**
		 * FunctionBar.executeAction Executes a language-specific defined action
		 * for a function button.
		 * 
		 */
		executeAction : function(definitionObject) {
			...
			var selText = FunctionBar
					.getFieldSelection($('#gollum-editor-body'));
			var repText = selText;
			var reselect = true;
			var cursor = null;
			...
			if (definitionObject.replace
				...
				if (repText === '') {
					...
					// find position of $1 - this is where we will place the
					// cursor
					cursor = rt.indexOf('$1');
		...
		/**
		 * getFieldSelectionPosition Retrieves the selection range for the
		 * textarea.
		 * 
		...
		getFieldSelectionPosition : function($field) {
			if ($field.length) {
				...
				if (typeof el.selectionStart == "number"
					...
					// so, uh, we're close, but we need to search for line
					// breaks and
					// adjust the start/end points accordingly since IE counts
					// them as
					...
					if (lb) {
						debug('IE start: compensating for ' + lb
								+ ' line breaks');
						start = start - lb;
						lb = 0;
					}
		...
		/**
		 * getFieldSelection Returns the currently selected substring of the
		 * textarea.
		 * 
		 * @param jQuery
		 *            A jQuery object for the textarea.
		...
		refresh : function() {
			if (EditorHas.functionBar()) {
				...
				if (LanguageDefinition.isValid()) {
					$('#gollum-editor-function-bar a.function-button').unbind(
							'click');
					FunctionBar.activate();
					if (Help) {
						Help.setActiveHelp(LanguageDefinition
								.getActiveLanguage());
					}
				} else {
		...
		/**
		 * replaceFieldSelection Replaces the currently selected substring of
		 * the textarea with a new string.
		 * 
		 * @param jQuery
		 *            A jQuery object for the textarea.
		 * @param string
		 *            The string to replace the current selection with.
		 * @param boolean
		 *            Reselect the new text range.
		 */
				...
				if ($field[0].setSelectionRange) {
					if (cursorOffset) {
						$field[0].setSelectionRange(
								selPos.start + cursorOffset, selPos.start
										+ cursorOffset);
					} else {
						$field[0].setSelectionRange(selPos.start, selPos.start
								+ replaceText.length);
					}
					...
					if (cursorOffset) {
						...
						range.moveEnd('character', selPos.start
								+ replaceText.length);
	...
	var FormatSelector = {
		...
		/**
		 * FormatSelector.evtChangeFormat Event handler for when a format has
		 * been changed by the format selector. Will automatically load a new
		 * language definition via JS if necessary.
		 * 
		 * @return void
		 */
		...
		/**
		 * FormatSelector.init Initializes the format selector.
		 * 
		 * @return void
		 */
		init : function($sel) {
	...
	var Help = {
		 ...
		 * Help.define
		 * 
		 * Defines a new help context and enables the help function if it exists
		 * in the Gollum Function Bar.
		 * 
		 * @param string
		 *            name The name you're giving to this help context.
		 *            Generally, this should match the language name.
		 * @param object
		 *            definitionObject The definition object being loaded from a
			...
			if (Help.isValidHelpFormat(definitionObject)) {
				...
				if ($("#function-help").length) {
					...
					if ($('#gollum-editor-help').length
							&& typeof $('#gollum-editor-help').attr(
									'data-autodisplay') !== 'undefined'
							&& $('#gollum-editor-help')
									.attr('data-autodisplay') === 'true') {
						Help.show();
					}
				}
			} else {
		...
		/**
		 * Help.generateHelpMenuFor Generates the markup for the main help menu
		 * given a context name.
		 * 
		 * @param string
		...
		/**
		 * Help.generateSubMenu Generates the markup for the inline help
		 * sub-menu given the data object for the submenu and the array index to
		 * start at.
		 * 
		 * @param object
		 *            subData The data for the sub-menu.
		 * @param integer
		 *            index The index clicked on (parent menu index).
		 * @return void
		 */
		...
		show : function() {
			if ($.browser.msie) {
				// bypass effects for internet explorer, since it does weird
				// crap
		...
		/**
		 * Help.showHelpFor Displays the actual help content given the two menu
		 * indexes, which are rendered in the rel="" attributes of the help
		 * menus
		 * 
		 * @param integer
		 *            index1 parent index
		 * @param integer
		 *            index2 submenu index
		 * @return void
		 */
		...
		/**
		 * Help.isLoadedFor Returns true if help is loaded for a specific markup
		 * language, false otherwise.
		 * 
		 * @param string
		 *            name The name of the markup language.
		 * @return boolean
		 */
		...
		/**
		 * Help.isValidHelpFormat Does a quick check to make sure that the help
		 * definition isn't in a completely messed-up format.
		 * 
		 * @param object
		 *            (Array) helpArr The help definition array.
		 * @return boolean
		 */
		isValidHelpFormat : function(helpArr) {
			return (typeof helpArr == 'object' && helpArr.length
					&& typeof helpArr[0].menuName == 'string'
					&& typeof helpArr[0].content == 'object' && helpArr[0].content.length);
		},

		/**
		 * Help.setActiveHelp Sets the active help definition to the one defined
		 * in the argument, re-rendering the help menu to match the new
		 * definition.
		 * 
		 * @param string
		 *            name The name of the help definition.
		 * @return void
		 */
		...
		/**
		 * Help.evtHelpButtonClick Event handler for clicking the help button in
		 * the function bar.
		 * 
		 * @param jQuery.Event
		...
		/**
		 * Help.evtParentMenuClick Event handler for clicking on an item in the
		 * parent menu. Automatically renders the submenu for the parent menu as
		 * well as the first result for the actual plain text.
		 * 
		 * @param jQuery.Event
		 *            e The jQuery event object.
		 * @return void
		...
		/**
		 * Help.evtSubMenuClick Event handler for clicking an item in a help
		 * submenu. Renders the appropriate text for the submenu link.
		 * 
		 * @param jQuery.Event
		 *            e The jQuery event object.
		 * @return void
